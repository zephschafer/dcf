...
are published via `pvc publish`, consumed from Kafka, projected through the pipeline schema, and
written as windowed Parquet files to the local warehouse. Then verify idempotency and clean
teardown via `pvc undeploy`.

**This is the first pvc scenario that requires zero external credentials.**

**The core questions this scenario answers:**
1. Does `pvc deploy` (with `catalog: local`) start a Kafka broker and stream runner?
2. Does `pvc publish` successfully deliver messages to the local Kafka topic?
...
5. Does `pvc undeploy` cleanly remove all Docker resources without touching warehouse data?

## Target Component

This scenario tests `pvc/local_deploy.py` (Docker orchestration), `pvc/local_stream_runner.py`
(Kafka consumer + windowed Parquet writer), and the `pvc publish` CLI command. No external API,
...
ignores it — the Kafka topic is always derived as `pvc-{pipeline_name}`. The subscription
...
4. Run `pvc validate click_events` — should show `(streaming, subscription: ..., 5 columns)`.
...
1. Run `pvc deploy click_events`.
   - Expect: Docker network `pvc-click_events` created.
   - Expect: Kafka container `pvc-kafka-click_events` started with KRaft (no Zookeeper).
   - Expect: Topic `pvc-click_events` created in Kafka.
   - Expect: Runner image `pvc-local/click_events-stream:latest` built.
   - Expect: Runner container `pvc-runner-click_events` started.
   - Note time for each step (Kafka ~5-10s; first image build ~1-2 min).
2. Run `docker ps` — confirm both `pvc-kafka-click_events` and `pvc-runner-click_events`
   show status `Up`.
3. Run `docker logs pvc-runner-click_events` — should contain "Local stream runner started".
4. Run `pvc deploy-status click_events` — should show `streaming (local Docker + Kafka)`,
   Kafka container name, topic `pvc-click_events`, external bootstrap `localhost:29092`, window 15s.
...
4. Run `pvc query "SELECT COUNT(*) FROM click_events.click_events"` — should return 10.

5. Run `pvc query "SELECT * FROM click_events.click_events LIMIT 5"` — spot-check all 5 columns.

6. Run `docker logs pvc-runner-click_events` — should contain "Wrote 10 rows to window-*.parquet".

Phase 3 success: 10 rows in warehouse, all 5 columns present and correctly typed.

### Phase 4 — Idempotency and Undeploy

1. Run `pvc deploy click_events` again — should tear down old containers and start fresh.
   Verify existing Parquet files in `warehouse/click_events/` are still present (undeploy
   does not touch warehouse data).
2. Run `pvc publish click_events '{"event_id":"evt-011","user_id":2,"action":"view","page":"/docs","timestamp":"2026-05-12T21:00:00Z"}' --count 3`.
3. Wait 20s — a new Parquet file appears (confirms the fresh deployment is working).
4. Run `pvc undeploy click_events --yes`.
5. Run `docker ps` — no `pvc-kafka-click_events` or `pvc-runner-click_events`.
6. Run `docker network ls | grep pvc-click_events` — network removed.
...
- [ ] Phase 1: `pvc validate click_events` passes with `(streaming, ...)` annotation
- [ ] Phase 1: `docker info` succeeds (Docker is running)
- [ ] Phase 2: `pvc deploy click_events` completes without error
- [ ] Phase 2: `docker ps` shows `pvc-kafka-click_events` with status `Up`
- [ ] Phase 2: `docker ps` shows `pvc-runner-click_events` with status `Up`
- [ ] Phase 2: `docker logs pvc-runner-click_events` contains "Local stream runner started"
- [ ] Phase 2: `pvc deploy-status click_events` shows `streaming (local Docker + Kafka)`
- [ ] Phase 3: `pvc publish click_events '...' --count 10` exits 0
...
- [ ] Phase 3: `docker logs pvc-runner-click_events` confirms rows were written
- [ ] Phase 4: Second `pvc deploy click_events` completes without error (idempotency)
- [ ] Phase 4: `pvc publish` and window flush work on the fresh deployment
- [ ] Phase 4: `pvc undeploy click_events --yes` completes without error
- [ ] Phase 4: `docker ps` shows no pvc containers; network removed; warehouse data intact
...
- **Dual-listener networking**: Kafka exposes two listeners — `INTERNAL` (for the runner
  container on the Docker network at `pvc-kafka-click_events:9092`) and `EXTERNAL` (for
...
- **First image build**: `pvc-local/click_events-stream:latest` is built fresh on first
...
- **`source.subscription` is required but unused locally**: The schema requires a
  ...
  (Kafka topic is `pvc-{pipeline_name}`). The placeholder path `projects/local-dev/...`
...
- **UX (possible)**: `window_seconds` cannot be overridden at deploy time without editing
  the pipeline YAML. A `--window-seconds N` flag on `pvc deploy` would improve local
...
- Full command prefix: `PVC_PROJECT_DIR=$CLONE uv --directory /Users/zephschafer/Documents/GitHub/pvc run pvc <command>`
...
- If both containers start but no Parquet appears after 30s, check `docker logs pvc-runner-click_events`
  for connection errors to Kafka — this points to a networking or timing issue.
- If `pvc query` fails to find the table, check that `warehouse_reader.py` is resolving the
