...
were not reachable because the pipeline YAML cannot even be loaded by pvc's schema.

## Success Criteria

- [ ] Phase 1: `pvc validate click_events` accepts `source.type: pubsub` and `deploy.type: streaming`
- [x] Phase 1: `pvc validate` rejects malformed YAML with a clear error message
- [x] Phase 1: `pvc validate` on an existing batch pipeline is unaffected (no regression)
...
- `pvc validate craigslist_apts` (batch regression): ✓ — no regression in batch validation
- `pvc validate click_events` malformed YAML: ✓ — rejects malformed column YAML with clear error
- `pvc deploy craigslist_apts` (no deploy block): ✓ — clear error: `'craigslist_apts' has no 'deploy:' block`
- `pvc deploy` with `catalog: local`: ✓ — clear error: `catalog is not 'gcp'. Batch deployment requires a GCP data lake.`

## What Failed

- `pvc validate click_events` — two validation errors:
...
- `pvc deploy click_events` — fails at model load due to F-037 and F-038 before any deploy logic runs
  [→ Finding F-039: Blocking / Runtime]

## Pre-identified Findings Confirmed but Not Directly Observed (Blocked Upstream)

- No `streaming_pipeline` Terraform module at `pvc/infra/modules/gcp/streaming_pipeline/`
  [→ Finding F-040: Blocking / Runtime — confirmed by codebase inspection]

- No Beam runner code in pvc (no Pub/Sub → GCS pipeline)
  [→ Finding F-041: Blocking / Runtime — confirmed by codebase inspection]

- `pvc undeploy` would call `terraform destroy` (cancel) not drain on a Dataflow job
...
See `pipeline.yml` in this directory — the intended pipeline YAML that pvc cannot yet
validate or deploy.
...
2. **F-039 (cli.py / batch_deploy.py):** `pvc deploy` must check `pipeline.deploy.type`
   and route to streaming deploy path when `type == "streaming"`.

3. **F-040 (new Terraform module):** Create `pvc/infra/modules/gcp/streaming_pipeline/`
...
4. **F-041 (new Beam runner):** Create streaming pipeline code (e.g.
   `pvc/gcp/streaming_deploy.py`) that builds a Beam pipeline:
   `ReadFromPubSub → json.loads → project_message → WindowInto(FixedWindows) → WriteToParquet(GCS)`

5. **F-042 (batch_deploy.py undeploy path):** For streaming pipelines, `pvc undeploy`
