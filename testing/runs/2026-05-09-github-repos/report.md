...
## Success Criteria
- [x] Pipeline YAML validates successfully (`ddt validate github_repos`)
...
- [~] Warehouse queryable via DuckDB — works, but query path is `warehouse/github_repos/github_repos/data/*.parquet`, not the `github.repos` shorthand in the scenario (no namespace concept in ddt today) [→ F-002]
...
During `ddt run`, ~20 lines of Spark/JVM/Ivy WARN messages appear before any ddt output. Example:
```
...
```
These warnings obscure the ddt progress output and are confusing for users who just want to ingest data. Especially jarring for a simple single-request HTTP pipeline that doesn't require Spark at all (incremental write uses PyArrow directly).
...
The `topics` field in GitHub API response is an array of strings: `["java", "tapestry", "web-framework"]`. ddt has no mechanism to handle array fields in the schema — dot-path returns the whole array as a Python list, which cannot be cast to any ddt type. Currently must be omitted from the schema.
...
## Proposed Fixes
1. F-001: Suppress Spark/Hadoop/Ivy WARN output; only show ddt lines during run. Options: configure Spark log4j to ERROR-only for infrastructure loggers, or redirect JVM stderr to /dev/null.
...
## Notes
- Test project used: `/Users/zephschafer/Documents/GitHub/quipu/` (ddt editable install from `../ddt`)
